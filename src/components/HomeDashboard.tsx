...
export const HomeDashboard: React.FC<HomeDashboardProps> = ({ onSelectTool }) => {
  ...
  const getToolIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Code2':
        return <Code2 className="w-6 h-6 text-violet-300" />;
      case 'Brush':
        return <Brush className="w-6 h-6 text-cyan-300" />;
      case 'KeyRound':
    ...
    <div className="space-y-10 pb-12">
        ...
        {filteredTools.map((tool) => {
          ...
          return (
            <div
              ...
              className={`group relative flex flex-col justify-between bg-slate-900/80 hover:bg-slate-900 border rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer overflow-hidden ${
                isMobile
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-12 h-12 rounded-xl border flex items-center justify-center group-hover:scale-110 transition-transform shadow-md ${
                    isMobile
                      ? 'bg-gradient-to-tr from-cyan-950 to-slate-900 border-cyan-500/30'
                      : isWeb
                      ? 'bg-gradient-to-tr from-violet-950 to-slate-900 border-violet-500/30'
                      : 'bg-gradient-to-tr from-slate-950 to-slate-900 border-slate-700'
                  }`}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {tool.badge && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                        {tool.badge}
                      </span>
                    )}
                  </div>
                ...
                <p className={`text-xs font-semibold mb-2 ${isMobile ? 'text-cyan-400/90' : isWeb ? 'text-violet-400/90' : 'text-slate-400'}`}>
                  {tool.titleTh}
                </p>
                  ...
                  {tool.features.map((feat, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-[11px] text-slate-300">
                      <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${
                        isMobile ? 'text-cyan-400' : isWeb ? 'text-violet-400' : 'text-indigo-400'
                      }`} />
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-800/60 text-xs font-semibold text-cyan-400 group-hover:text-cyan-300">
                ...
                <div className={`flex items-center justify-center w-7 h-7 rounded-lg bg-slate-800 text-slate-300 group-hover:text-white transition-all ${
      ...
      <section className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <h4 className="text-xs font-bold text-slate-200">SonarQube Certified</h4>
          <p className="text-[11px] text-slate-400">
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="p-2.5 rounded-xl bg-violet-950/60 border border-violet-500/30 text-violet-400">
            <Cpu className="w-5 h-5" />
          </div>
          <h4 className="text-xs font-bold text-slate-200">Next.js &amp; Flutter Ready</h4>
          <p className="text-[11px] text-slate-400">
