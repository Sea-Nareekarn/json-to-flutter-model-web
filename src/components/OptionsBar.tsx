...
export const OptionsBar: React.FC<OptionsBarProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 p-4 lg:p-5 rounded-2xl shadow-sm transition-colors space-y-4">
      ...
      {/* Second Row: Null Safety & Feature Chips */}
      <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex flex-col lg:flex-row lg:items-center justify-between gap-3 text-xs">
        {/* Null Safety Selector */}
        <div className="flex items-center gap-2.5 flex-shrink-0">
          <span className="text-slate-600 dark:text-slate-400 font-semibold flex items-center gap-1.5 h-8">
            <ShieldCheck className="w-4 h-4 text-emerald-500" />
            Null Safety:
          </span>
          <div className="inline-flex bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800 h-8 items-center">
            {nullabilityModes.map((nm) => {
              ...
              return (
                <button
                  ...
                  className={`h-6 px-2.5 rounded-lg transition-all font-medium text-xs flex items-center justify-center ${
                    isSelected
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <span className="text-slate-600 dark:text-slate-400 font-semibold flex items-center gap-1.5 flex-shrink-0 h-8">
            ...
            {featureToggles.map((item) => {
              ...
              return (
                <button
                  ...
                  title={item.desc}
                  className={`h-8 px-3 rounded-xl text-xs font-semibold inline-flex items-center gap-1.5 transition-all border ${
                    isOn
