...
export const OptionsBar: React.FC<OptionsBarProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 p-4 lg:p-5 rounded-2xl shadow-sm transition-colors space-y-4">
      {/* Top row: Root Class Name & Generator Model Style */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
        {/* Root Class Name */}
        <div className="md:col-span-4 flex flex-col">
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-1.5 h-4">
            ...
            {styles.map((st) => {
              ...
              return (
                <button
                  ...
                  className={`h-[52px] flex flex-col justify-center items-start px-3 py-1.5 rounded-xl border text-left transition-all ${
                    isActive
                      ? 'bg-cyan-50 dark:bg-cyan-950/40 border-cyan-500 text-cyan-800 dark:text-cyan-200 ring-1 ring-cyan-500/50 shadow-sm'
                ...
                >
                  <span className="text-xs font-bold flex items-center justify-between w-full">
                    <span>{st.label}</span>
                    {isActive && <Check className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400 flex-shrink-0 ml-1" />}
                  </span>
                  <span className="text-[10px] text-slate-500 line-clamp-1 mt-0.5">{st.tag}</span>
                </button>
              );
            })}
          </div>
      ...
      {/* Second Row: Null Safety & Feature Chips matching 4-col / 8-col grid */}
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800/80 grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
        {/* Null Safety Selector */}
        <div className="md:col-span-4 flex flex-col">
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-1.5 h-4">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" />
            Null Safety Mode
          </label>
          <div className="inline-flex bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800/80 h-[42px] items-center w-full">
            {nullabilityModes.map((nm) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 h-8 rounded-lg transition-all font-semibold text-xs flex items-center justify-center ${
                    isSelected
            ...
            {featureToggles.map((item) => {
              ...
              return (
                <button
                  ...
                  title={item.desc}
                  className={`h-[42px] px-3.5 rounded-xl text-xs font-semibold inline-flex items-center gap-2 transition-all border ${
                    isOn
                      ? 'bg-cyan-50 dark:bg-cyan-950/50 border-cyan-500 text-cyan-800 dark:text-cyan-200 shadow-sm ring-1 ring-cyan-500/40'
                      : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700/60 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800'
                  }`}
                >
                  <span
                    className={`w-2 h-2 rounded-full flex-shrink-0 ${
                      isOn ? 'bg-cyan-500 dark:bg-cyan-400 ring-2 ring-cyan-500/30' : 'bg-slate-300 dark:bg-slate-600'
                    }`}
                  />
