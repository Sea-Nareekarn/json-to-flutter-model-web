...
import { Minimize2, Trash2, Copy, Check, Upload, AlertCircle, FileCode, AlignLeft } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900/70 rounded-xl border border-slate-200 dark:border-slate-800 shadow-md dark:shadow-xl overflow-hidden transition-colors">
      ...
      <div className="relative flex-1 min-h-[450px]">
        <textarea
          ...
          spellCheck={false}
          className="w-full h-full p-4 bg-slate-50 dark:bg-slate-950/90 text-slate-900 dark:text-slate-200 font-mono text-xs sm:text-sm leading-relaxed resize-none focus:outline-none focus:ring-1 focus:ring-cyan-500/30 selection:bg-cyan-500/30"
        />

        {/* JSON Syntax Error Overlay Banner */}
        {error && (
          <div className="absolute bottom-3 left-3 right-3 p-3 rounded-lg bg-rose-50 dark:bg-rose-950/90 border border-rose-300 dark:border-rose-600/60 text-rose-800 dark:text-rose-200 text-xs flex items-start gap-2 shadow-lg backdrop-blur-sm animate-pulse-subtle">
            <AlertCircle className="w-4 h-4 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
            <div className="flex-1">
              <div className="font-semibold text-rose-700 dark:text-rose-300">Invalid JSON Syntax</div>
              <div className="font-mono text-[11px] text-rose-600 dark:text-rose-200/90 mt-0.5 line-clamp-2">
                {error}
              </div>
            </div>
          </div>
